...
| 预留填充基差 `mid_quote` / `vwap_window` / `twap_window` | `twap_window` 已实现（§6），其余仍为 NotImplementedError |
...
**文件**: `src/model/types.rs`, `src/execution/common.rs`, `src/execution/twap.rs`, `src/engine/python.rs`
...
| `ohlc4` | OHLC 均价 | ✅ 已有 |
| `hl2` | (H+L)/2 | ✅ 已有 |
| `mid_quote` | (H+L)/2 中间价 | ✅ 已有 |
| `typical` | (H+L+C)/3 典型价 | ✅ 已有 |
| `vwap_bar` | 单 bar VWAP 近似 | ✅ 已有 |
| `twap_window` | 多 bar 时间加权拆单 | ✅ 已实现（TwapScheduler 均匀拆单、后置截断、per-order 支持） |
...
**已有**: 103 个 Rust 后端指标函数 + 60 个 K 线形态（60 个已实现）
...
**已扩展 — K 线形态识别（Batch 2, 10 个）**:

| 形态 | 说明 | 返回值 |
|------|------|--------|
| `CDLPIERCING` | 刺透形态 | +100 |
| `CDLDARKCLOUDCOVER` | 乌云盖顶 | -100 |
| `CDLHARAMICROSS` | 十字孕线 | +100/-100 |
| `CDLMARUBOZU` | 光头光脚 | +100/-100 |
| `CDLKICKING` | 跳空形态 | +100/-100 |
| `CDLSPINNINGTOP` | 纺锤线 | +100/-100 |
| `CDLRISEFALL3METHODS` | 上升/下降三法 | +100/-100 |
| `CDLTHRUSTING` | 插入形态 | -100 |
| `CDLINNECK` | 颈内线 | -100 |
| `CDLONNECK` | 颈上线 | -100 |

**已扩展 — K 线形态识别（Batch 3, 10 个）**:

| 形态 | 说明 | 返回值 |
|------|------|--------|
| `CDLTRISTAR` | 三星形态 | +100/-100 |
| `CDL3LINESTRIKE` | 三线打击 | +100/-100 |
| `CDLADVANCEBLOCK` | 前进受阻 | -100 |
| `CDLTASUKIGAP` | 田子缺口 | +100/-100 |
| `CDLIDENTICAL3CROWS` | 同构三鸦 | -100 |
| `CDLBREAKAWAY` | 脱离形态 | +100/-100 |
| `CDLCONCEALBABYSWALL` | 藏婴吞没 | +100 |
| `CDLMATHOLD` | 铺垫形态 | +100/-100 |
| `CDLSEPARATINGLINES` | 分离线 | +100/-100 |
| `CDLXSIDEGAP3METHODS` | 跳空三法 | +100/-100 |

**已扩展 — K 线形态识别（Batch 4, 10 个）**:

| 形态 | 说明 | 返回值 |
|------|------|--------|
| `CDL3STARSINSOUTH` | 南方三星 | +100 |
| `CDLCOUNTERATTACK` | 反击线 | +100/-100 |
| `CDLGAPSIDESIDEWHITE` | 跳空并列阳线 | +100/-100 |
| `CDLHOMINGPIGEON` | 归巢鸽 | +100 |
| `CDLLADDERBOTTOM` | 梯底 | +100 |
| `CDLMATCHINGLOW` | 匹配低点 | +100 |
| `CDLRICKSHAWMAN` | 黄包车夫 | +100/-100 |
| `CDLTAKURI` | 探底线 | +100 |
| `CDLUNIQUE3RIVER` | 独特三河底 | +100 |
| `CDLMATCHINGHIGH` | 匹配高点 | -100 |

**已扩展 — K 线形态识别（Batch 5, 10 个, 完结）**:

| 形态 | 说明 | 返回值 |
|------|------|--------|
| `CDLABANDONEDBABY` | 弃婴形态 | +100/-100 |
| `CDLDOJISTAR` | 十字星 | +100/-100 |
| `CDLEVENINGDOJISTAR` | 暮星十字 | -100 |
| `CDLHIGHWAVE` | 高浪线 | +100/-100 |
| `CDLHIKKAKE` | 匹卡克形态 | +100/-100 |
| `CDLINVERTEDHAMMER` | 倒锤线 | +100 |
| `CDLKICKINGBYLENGTH` | 跳空（按长度） | +100/-100 |
| `CDLLONGLEGGEDDOJI` | 长腿十字 | +100 |
| `CDLMORNINGDOJISTAR` | 晨星十字 | +100 |
| `CDLSTICKSANDWICH` | 棒三明治 | +100 |

**已扩展 — 周期指标（希尔伯特变换, 3 个）**:

| 指标 | 说明 | 输出 |
|------|------|------|
| `HT_DCPERIOD` | 主导周期（Ehlers 自适应分解） | 周期（bars） |
| `HT_DCPHASE` | 主导相位（Ehlers 自适应分解） | 相位角（0-360°） |
| `HT_PHASOR` | 相量分量（同相/正交） | (in_phase, quadrature) 元组 |
...
| `TensorFlowAdapter` | TensorFlow/Keras 模型（lazy import），增量与非增量训练，save/load |
...
Phase 2 (P1) ── Gateway 收口 + 资产类模块 + 期权风控 + 填充策略
  ├── 5.1 Gateway 契约与边界收口 ✅
  ├── 3.x futures/stock/option/fund Python 封装 ✅
  ├── 4.x 期权 Greek 风控 ✅ (BSM/Greeks/IV/保证金/per-slot)
  ├── 6.x 填充策略实现 ✅ (mid_quote/typical/vwap_bar/twap_window 全部完成)
...
Phase 3 (P2) ── 扩展能力 ✅ 已完成
  ...
  ├── 8.x TA-Lib 指标扩展 ✅ Batch 1-5 (60 CDL, 完结) + Hilbert 3
  ├── 9.x Indicator 增量更新 ✅
  └── 10.x ML 适配器扩展 ✅ (LightGBM/XGBoost/TensorFlow)

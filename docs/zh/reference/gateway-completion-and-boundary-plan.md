...
原始方案针对 akquant gateway 层的三个功能缺口，同时为 miniQMT 桥接层未来迁出做准备：

1. **市场模型硬编码为期货** — 原始问题是 `LiveRunner.run()` 无条件调用 `use_china_futures_market()`，股票类 broker（miniqmt、ptrade）错误使用 T+0 规则
2. **broker_live 注入的 `submit_order` 签名与基类不一致** — 原始问题是基类有 `broker_options` 参数，但 broker_live 注入闭包缺少该参数；策略按文档传 `broker_options=` 会直接 TypeError
3. **miniQMT 桥接边界未标记** — 原始问题是 `miniqmt_xtquant.py` 无任何迁移提示，`factory.py` 未准备 `bridge_url` 入口

当前工作树里，Change 1/2/3 的核心代码路径已经基本落地；本文档现在更适合作为“剩余收口项”说明，重点包括：

- 补齐测试覆盖，使实现与预期 fail-closed 语义一致
- 同步参考文档，避免 API / Gateway 文档继续描述旧现状
- 明确 miniQMT 迁移边界，避免把当前 xtquant 直连误解为长期架构
...
**当前代码状态**：该核心实现已在当前工作树中落地，`factory.py` 已补 `asset_class` metadata，`LiveRunner.run()` 已在 `create_gateway_bundle()` 之后调用 `_select_market_model(bundle)`。本 Change 的剩余工作主要是测试与参考文档同步。
...
**当前代码状态**：`UnifiedOrderRequest.broker_options`、broker_live 注入签名补齐、`extra` / 高级参数的 fail-closed 分支，当前工作树中已基本存在。剩余需要确认的是：

- 是否补上对未支持 `order_type` 的显式限制，避免下游把 `StopTrail` 等值误当成普通限价单
- 测试是否完整覆盖 `broker_options` 正常路径，以及 `extra` / 高级参数 / 未支持 `order_type` 的拒绝路径
- Gateway 系统参考文档是否同步加入 `UnifiedOrderRequest.broker_options`
...
**当前代码状态**：`bridge_url` guard 与构造时 `FutureWarning` 已在当前工作树中出现。剩余工作主要是测试补齐，以及把迁移提示同步到模块 docstring / 参考文档中，避免边界说明只存在于运行时 warning。
...
| # | 标题 | 核心文件 |
|---|------|---------|
| 1 | `test(live): cover market model selection by broker asset_class` | test_gateway_factory.py, test_live_runner_broker_bridge.py |
| 2 | `test(gateway): cover broker_live broker_options and fail-closed semantics` | test_live_runner_broker_bridge.py, test_gateway_miniqmt_xtquant.py |
| 3 | `docs(gateway): sync UnifiedOrderRequest and miniqmt migration boundary docs` | docs/zh/reference/gateway_system.md, docs/zh/reference/gateway-completion-and-boundary-plan.md, tests if needed |

如果后续发现 Change 2 仍缺 `order_type` 显式限制，可在第 2 个提交前先补实现，再按同一批次补对应测试。
